...
7-config matrix (per `README.md`):

    --config lru          # no HiMA
    --config l1_only      # VLLM_HIMA_L1_ENABLE=1
    --config l2_only      # VLLM_HIMA_L2_ENABLE=1
    --config full         # L1 + L2
    --config l1_pcache    # L1 + partial-cache
    --config l2_pcache    # L2 + partial-cache
    --config full_pcache  # L1 + L2 + partial-cache (Songyang's prod config
                          #   modulo hybrid → single-group)
...
_CONFIG_ENV = {
    "lru": {},
    "pcache": {"VLLM_PARTIAL_CACHE_ENABLED": "1"},  # pcache alone, no HiMA
    # Diagnostics for the pcache regression:
    "pcache_probe_only": {  # insert path runs, lookup runs, but extension NOT applied
        "VLLM_PARTIAL_CACHE_ENABLED": "1",
        "VLLM_PARTIAL_CACHE_PROBE_ONLY": "1",
    },
    "pcache_insert_off": {  # lookup-only; insert path disabled
        "VLLM_PARTIAL_CACHE_ENABLED": "1",
        "VLLM_PARTIAL_CACHE_INSERT_OFF": "1",
    },
    "pcache_force_miss": {  # both paths run, but all lookups forced to miss
        "VLLM_PARTIAL_CACHE_ENABLED": "1",
        "VLLM_PARTIAL_CACHE_FORCE_MISS": "1",
    },
...
def apply_env(config: str) -> None:
    ...
    for k in ("VLLM_HIMA_L1_ENABLE", "VLLM_HIMA_L2_ENABLE", "VLLM_PARTIAL_CACHE_ENABLED"):
...
def main() -> int:
    ...
    fout.write(json.dumps({
        ...
        "env": {k: os.environ.get(k, "") for k in (
            ...
            "VLLM_PARTIAL_CACHE_ENABLED", "VLLM_HIMA_HPB_WINDOW_S",
        )},
    }) + "\n")
    fout.flush()
